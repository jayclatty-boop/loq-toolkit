...
public static partial class Compatibility
{
    ...
    // LOQ-specific model prefixes
    private static readonly string[] LOQModelsPrefix = [
        // LOQ 15 series
        "15IAX",
        "15IRX",
        "15AHP",
    ...
    /// <summary>
    /// Checks if the current device is a Lenovo LOQ model.
    /// </summary>
    public static async Task<bool> IsLOQModelAsync()
    {
        var mi = await GetMachineInformationAsync().ConfigureAwait(false);
        return IsLOQModel(mi.Model, mi.MachineType);
    }

    /// <summary>
    /// Checks if the specified model/machine type is a LOQ model.
    /// </summary>
    public static bool IsLOQModel(string model, string machineType)
    {
        foreach (var loqPrefix in LOQModelsPrefix)
        {
            if (model.Contains(loqPrefix, StringComparison.InvariantCultureIgnoreCase))
                return true;
            if (machineType.Contains(loqPrefix, StringComparison.InvariantCultureIgnoreCase))
                return true;
        }
        return false;
    }
